    ...
    async buildPage(callback = () => {}){
        ...
        SCROLL_TRACKER.addHandler('set', async (ss, delta_ss) => {
            ...
            if(ss[0] == this.par_id){
                ...
                if (this.child_id == this.ss_map[this.par_id]){   //&& this.load_count > 1 (if you want scroll_handler to not launch on first scroll)
                    ...
                    var timeout_ = this.load_count == 1 ? 300: 0
                    setTimeout(() => {
                        this.scroll_handler(pd, ss, delta_ss);
                    }, timeout_)
                }
