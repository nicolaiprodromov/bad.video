    ...
    async glitchTransition(res){
        ...
        await this.forEach(
            ...
            (instance, data, pixel_data) => {
                setTimeout(() => {
                    let temp_img = this.img
                    temp_img.style.filter = "invert(100%)"
                    ctx.drawImage(temp_img,   data.x*rr_tw, data.y*rr_th, data.sx*rr_tw*((Math.random() + .5) * .75), data.sy*rr_th*((Math.random() + .5) * .75),   data.x, data.y, data.sx, data.sy)
                }, 100 + (data.index*30)/4)
                setTimeout(() => {
